...
import { X, Home, ShoppingCart, PlusCircle, MessageCircle, User, Settings, HelpCircle, HeadphonesIcon, HistoryIcon, PackagePlus, Leaf } from 'lucide-react';
...
const MenuItem = ({ icon: Icon, label, to, onClick }: { icon: any; label: string; to: string; onClick: () => void }) => {
  ...
  return (
    ...
    >
      <div className={`${isActive ? 'text-white' : 'text-green-500 group-hover:text-green-600'}`}>
        <Icon className="w-5 h-5" />
      </div>
      <span className={`font-medium ${isActive ? '' : 'group-hover:translate-x-1'} transition-transform duration-200`}>
        {label}
      </span>
      {isActive && (
      ...
      >
        ...
        <nav className="py-5 space-y-1.5 flex-1 overflow-y-auto">
          ...
          <div className="px-6 py-3">
            ...
            <p className="text-xs text-gray-500 uppercase tracking-wider mt-3 mb-1 px-2">Farmer</p>
          </div>
          
          <MenuItem icon={PackagePlus} label="Orders Recieved" to="/Admin-orders" onClick={onClose} />
          <MenuItem icon={MdInventory} label="Products Listed" to="/Admin_products" onClick={onClose} />
          
          <div className="px-6 py-3">
            <div className="border-t border-gray-200"></div>
            <p className="text-xs text-gray-500 uppercase tracking-wider mt-3 mb-1 px-2">Settings</p>
          </div>
        ...
        <div className="p-5 mt-auto">
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4">
            ...
            <p className="text-xs text-green-600 mt-1">Contact our support team</p>
