...
const Dashboard = () => {
  ...
  const recentOrders = [
    ...
    { id: "ORD-003", customer: "Mike Johnson", amount: "$499.99", status: "processing" }
  ];
        ...
        {stats.map((stat, index) => (
          <div key={index} className="bg-white p-6 rounded-lg shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <span className={`flex items-center text-sm ${stat.increasing ? 'text-green-600' : 'text-red-600'}`}>
                {stat.change}
                {stat.increasing ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
              </span>
            </div>
